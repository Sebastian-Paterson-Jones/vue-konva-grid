...
export const getItemMetadata = ({
  ...
  const itemMetadataMap = isColumn ? instanceProps.columnMetadataMap : instanceProps.rowMetadataMap;
  
  // Early return if index is already calculated
  if (index in itemMetadataMap) {
    return itemMetadataMap[index];
  }
  
  const lastMeasuredIndex = isColumn ? instanceProps.lastMeasuredColumnIndex : instanceProps.lastMeasuredRowIndex;
  
  // If index is within already measured range but not in map (can happen with sparse arrays), compute it
  if (index <= lastMeasuredIndex) {
    return itemMetadataMap[index];
  }
  
  const itemSize = isColumn ? columnWidth : rowHeight;
  const recalcIndices = isColumn ? instanceProps.recalcColumnIndices : instanceProps.recalcRowIndices;
  const recalcWithinBoundsOnly = recalcIndices.length > 0;
  
  // Calculate starting offset
  let offset = 0;
  if (lastMeasuredIndex >= 0) {
    const lastItemMetadata = itemMetadataMap[lastMeasuredIndex];
    offset = lastItemMetadata.offset + lastItemMetadata.size;
  }

  // Pre-multiply scale for performance
  const scaledSize = (idx: number) => itemSize(idx) * scale;
  
  // Calculate indices only up to the requested index, not all at once
  for (let i = lastMeasuredIndex + 1; i <= index; i++) {
    // Only recalculates specified columns
    let size;
    if (recalcWithinBoundsOnly) {
      if (recalcIndices.includes(i)) {
        size = scaledSize(i);
      } else {
        size = itemMetadataMap[i]?.size || scaledSize(i);
      }
    } else {
      size = scaledSize(i);
    }
